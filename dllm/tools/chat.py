...
def decode_trim(tokenizer, seq_ids: List[int], input_ids) -> str:
    """
...
def single_turn_generate(generator, gen_config, visualize: bool):
    ...
    while True:
        ...
        text = decode_trim(tokenizer, outputs.sequences[0].tolist(), inputs.tolist())
...
def multi_turn_chat(generator, gen_config, visualize: bool):
    ...
    while True:
        ...
        inputs = build_chat_inputs(tokenizer, messages, add_generation_prompt=True).to(model.device)

        outputs = generator.generate([inputs], gen_config, return_dict_in_generate=True)
        reply = decode_trim(tokenizer, outputs.sequences[0].tolist(), inputs.tolist())
