...
def test():
    ...
    # Parse CLI args (or use defaults if none provided)
    ...
    def test_generate():
        ...
        print("\n" + "=" * 80)
        print("TEST: model.diffusion_generate()".center(80))
        print("=" * 80)
        ...
        gt_out = model.diffusion_generate(
            inputs=input_ids,
        ...
        # remove left padding <eos_tokens> and tokens after the final <eos_token>
        generations = [g.lstrip("<|endoftext|>").split(tokenizer.eos_token, 1)[0] for g in tokenizer.batch_decode(gt_out)]
    ...
    def test_fill_in_blanks():
        ...
        out = fill_in_blanks(
            model=model,
            tokenizer=tokenizer,
            inputs_with_blanks=input_ids_list,
            steps=script_args.steps,
            temperature=script_args.temperature,
            top_p=script_args.top_p,
            top_k=script_args.top_k,
        ...
        for i, (ids, f) in enumerate(zip(input_ids_list, filled), 1):
            print("\n" + "-" * 80)
            print(f"[Case {i}]")
            print("-" * 80)
