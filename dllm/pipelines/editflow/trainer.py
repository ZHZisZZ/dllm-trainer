...
class EditFlowTrainer(transformers.Trainer):
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        w = w.clamp(max=self.max_w)  # TODO: prevent extreme weights, only needed when applying scaling factor to survival loss
        ...
        mask_f = x_mask.float()
        Lambda_hat = (
            (sub_rate_hat * mask_f).sum(dim=1) +
            (del_rate_hat * mask_f).sum(dim=1) +
            (ins_rate_hat * mask_f).sum(dim=1)   # insertions are defined per row incl. BOS row
        )  # [B]
        # TODO: whether we need scaling here
        loss_surv = (w * Lambda_hat).mean()

        # -------- 7) Positive edit terms --------
        # For each remaining edit e:  -log true rate(e)  - log token prob(e) if tokenized
        loss_pos = x_tok.new_tensor(0.0, dtype=torch.float32, device=device)
        for b, edits in enumerate(edits_list):
            if not edits:
                continue
            cur_len = int(x_mask[b].sum().item())  # number of rows (incl. BOS)
            wb = w[b]
            for e in edits:
                pos = e.pos
                assert 0 <= pos < cur_len, f"pos {pos} out of range {cur_len}"
                if e.kind == "SUB":
                    loss_pos = loss_pos - wb * (
                        logQ_sub[b, pos, e.token] + safe_log(sub_rate_hat[b, pos])
                    )
                elif e.kind == "DEL":
                    loss_pos = loss_pos - wb * safe_log(del_rate_hat[b, pos])
                else:  # "INS"
                    loss_pos = loss_pos - wb * (
                        logQ_ins[b, pos, e.token] + safe_log(ins_rate_hat[b, pos])
                    )
