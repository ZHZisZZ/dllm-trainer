...
class PowerKappaScheduler(BaseKappaScheduler):
    n: float = 1.0  # exponent; can be float
    ...
    def _kappa_derivative(self, i: torch.Tensor) -> torch.Tensor:
        # κ'(t) = n * t^(n-1)
        return self.n * (i ** (self.n - 1))


@dataclasses.dataclass
class LinearKappaScheduler(PowerKappaScheduler): n: float = 1.0
@dataclasses.dataclass
class QuadraticKappaScheduler(PowerKappaScheduler): n: float = 2.0
@dataclasses.dataclass
class CubicKappaScheduler(PowerKappaScheduler): n: float = 3.0
