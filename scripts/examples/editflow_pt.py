...
class TrainingArguments(transformers.TrainingArguments):
    output_dir: str = "models/EditFlow-Dream-7B"
    report_to: str = "wandb"
    ...
    scheduler_cls: str = "CubicKappaScheduler"
...
def init_editflow_from_dream(ef_model, dream_model, lm_head_key="lm_head", verbose=True):
    ...
    if verbose:
        print(f"[EditFlow init] Copied {len(new_sd)} tensors from DreamModel.")
        if missing:
            print("  Missing (expected for new rate heads, etc.):")
            for k in missing:
                print("   -", k)
        if unexpected:
            print("  Unexpected (check key names):")
            for k in unexpected:
                print("   -", k)
...
def train():
    ...
    # initialize model from dream_model (copies backbone + clones lm_head to both logits heads)
    ...
    ################
    # TODO: only for debug
    def train_map_fn_debug(
        row, 
        tokenizer: transformers.PreTrainedTokenizer, 
    ) -> dict:
        input_ids = tokenizer(row["messages"][-1]["content"], add_special_tokens=False)["input_ids"]
        import random
        len = random.randint(5, 16)
        input_ids = [tokenizer.bos_token_id] + input_ids[:len] + [tokenizer.eos_token_id]
        return {
            "input_ids": input_ids,
        }
    ...
    trainer = editflow.EditFlowTrainer(
        ...
        args=training_args,
        data_collator=editflow.utils.EditFlowCollator(tokenizer),
